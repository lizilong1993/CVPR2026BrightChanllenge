...
class Inference:
    def __init__(self, args):
        ...
        # Load model
        self.model = SiamAttnUNet(in_channels=3, num_classes=num_classes)
    ...
    def apply_tta_inference(self, model, pre_change_imgs, post_change_imgs):
        ...
        # 1) No transform
        output_clf = model(pre_change_imgs, post_change_imgs)  # output_clf is [B, num_damage_classes, H, W]
    ...
    def run_inference(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(tqdm(self.test_loader)):
                ...
                # Predict
                output_clf = self.model(pre_change_imgs, post_change_imgs)
