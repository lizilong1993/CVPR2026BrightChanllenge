    ...
    def run_inference(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(tqdm(self.test_loader)):
                ...
                # output = torch.argmax(fused_logits, dim=1).squeeze().cpu().numpy().astype(np.uint8)

                
                output = torch.argmax(output_clf, dim=1).squeeze().cpu().numpy().astype(np.uint8) # * output_loc
