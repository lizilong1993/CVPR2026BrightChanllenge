...
class Trainer(object):
    ...
    def training(self):
        ...
        for _ in tqdm(range(elem_num)):
            ...
            if (itera + 1) % 10 == 0:
                ...
                if (itera + 1) % 500 == 0:
                    ...
                    sum_mIoU = (val_mIoU + val_mIoU_fs) / 2
                    ...
                    if sum_mIoU > best_mIoU_sum:
                        torch.save(self.deep_model.state_dict(), os.path.join(self.model_save_path, f'best_model_sum.pth'))
                        best_mIoU_sum = sum_mIoU
                        best_round_sum = {
                            'Test event': self.args.test_event_name,
                            'best iter': itera + 1,
                            'best mIoU': [val_mIoU * 100, val_mIoU_fs * 100, test_mIoU * 100],
                            'best OA': [val_OA * 100, val_OA_fs * 100, test_OA * 100],
                            'best sub class IoU': (val_IoU_of_each_class * 100, val_IoU_of_each_class_fs * 100, test_IoU_of_each_class * 100)
                        }
        ...
        print('The accuracy of the best round selected using Both domain is ', best_round_sum)
...
def main():
    ...
    print(f'Train data (source domain) is {args.train_data_name_list}')
    print(f'Train data (target domain) is {new_train_data_name_list_few_shot}')
